...
type CallGraphTests(output: ITestOutputHelper) =
    ...
    let makeNode name specKind (paramRes: _ list) =
        ...
        let res =
            paramRes.ToImmutableDictionary((fun kvp -> (qualifiedName, fst kvp)), (snd >> ResolvedType.New))
    ...
    member this.``Concrete Graph Entry Points and Unit Tests``() =
        ...
        let fooString = makeNodeA "Foo" String
        let fooEmpty = makeNodeNoRes "Foo"
        let barDouble = makeNodeA "Bar" Double
        let barString = makeNodeA "Bar" String
        let barEmpty = makeNodeNoRes "Bar"
