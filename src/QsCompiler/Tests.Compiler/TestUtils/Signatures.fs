...
let private makeSignatures sigs =
    sigs |> Seq.map (fun (types, case) -> Seq.map (fun s -> makeSig s types) case) |> Seq.toArray
...
let public SignatureCheck checkedNamespaces targetSignatures compilation =
    ...
    let doesCallMatchSig call signature =
        let (callFullName: QsQualifiedName), callArgType, callReturnType = call
        let (sigFullName: QsQualifiedName), sigArgType, sigReturnType = signature

        callFullName.Namespace = sigFullName.Namespace
        && callFullName.Name.EndsWith sigFullName.Name
        && callArgType = sigArgType
        && callReturnType = sigReturnType
    ...
    for targetSig in targetSignatures do
        let sigFullName, sigArgType, sigReturnType = targetSig
    ...
    for callSig in callableSigs do
        let sigFullName, sigArgType, sigReturnType = callSig
